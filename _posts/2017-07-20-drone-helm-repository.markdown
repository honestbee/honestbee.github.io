...
A Chart repository mainly consists of static assets (Although a proposal exists to treat Charts more like Container images and [re-use the existing technology used to manage image distribution](https://coreos.com/blog/quay-application-registry-for-kubernetes.html)). Any static file host (github.io / s3 / GCS / ... ) can be used, additionaly - support for [auth has recently been added](https://github.com/kubernetes/helm/issues/1038) to Helm. We just have to have a pipeline to package the charts and synchronise them to the host for every change.

Next, our Drone pipeplines must have a task to pull the specified Helm Chart from the Chart Repository and install or upgrade a Project's target release (Dev / Staging / Prod) within a Kubernetes cluster. For these type of pipeline tasks, which are highly re-usable across pipelines, Drone uses the concept of a [plugin](http://docs.drone.io/plugin-overview/) making them fully declarative and easily controlled through configuration. 

A powerfull [Helm plugin for Drone](https://github.com/ipedrazas/drone-helm) already existed, but lacked the support to work with centralized Chart Repositories. We recently forked and added this support and are working to upstream these changes.
...
After the storage service has been configured, we will cover how Drone is used to synchronise our Git repositories with our Helm Repository.
